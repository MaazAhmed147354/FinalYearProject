...
def parse_pdf_to_cv_data(pdf_path):
    ...
    try:
        ...
        # Ensure all strings are properly processed to avoid encoding issues
        for key in sections:
            if isinstance(sections[key], str):
                sections[key] = sections[key].strip()
            elif isinstance(sections[key], list):
                if all(isinstance(item, str) for item in sections[key]):
                    sections[key] = [item.strip() for item in sections[key]]
