...
exports.syncEmails = async (config) => {
  try {
    ...
    const results = await connection.search(emailConfig.searchCriteria, emailConfig.fetchOptions);
    
    // Process emails
    const emails = [];
    
    for (const result of results) {
      const parts = result.parts.filter(part => part.which !== '');
      let emailObj = await simpleParser(parts[0].body);
      
      // Create email record
      const email = await Email.create({
        sender: emailObj.from.text,
        subject: emailObj.subject,
        body: emailObj.text || emailObj.html,
        received_date: emailObj.date,
      ...
      if (email.has_resume) {
        ...
        for (const attachment of emailObj.attachments) {
          const fileName = `${uuidv4()}_${attachment.filename.replace(/\s+/g, '_')}`;
          const filePath = path.join(uploadDir, fileName);
          
          // Save attachment
          await writeFile(filePath, attachment.content);
          
          // Add attachment path to email
          email.attachment_paths = email.attachment_paths || [];
          email.attachment_paths.push(filePath);
        }
        
        // Update email with attachment paths
        await email.update({ attachment_paths: JSON.stringify(email.attachment_paths) });
      }
...
exports.extractResumeFromEmail = async (emailId, jobId) => {
  try {
    ...
    const attachmentPaths = JSON.parse(email.attachment_paths || '[]');
...
exports.sendEmail = async (emailData) => {
  try {
    ...
    // Save sent email (optional, depending on requirements)
    // ...
