...
CREATE TABLE `reports` (
  ...
  `type` enum('shortlisted','rejected') NOT NULL,
  `content` text NOT NULL,
  `generated_by` int NOT NULL,
  `sent` tinyint(1) NOT NULL DEFAULT '0',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `fk_report_resume_id` (`resume_id`),
  KEY `fk_report_generated_by` (`generated_by`),
  CONSTRAINT `fk_report_generated_by` FOREIGN KEY (`generated_by`) REFERENCES `users` (`id`),
  CONSTRAINT `fk_report_resume_id` FOREIGN KEY (`resume_id`) REFERENCES `resumes` (`id`) ON DELETE CASCADE
